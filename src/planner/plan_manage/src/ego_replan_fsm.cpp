...
namespace ego_planner
{
  ...
  void EGOReplanFSM::plotfigure(const ros::TimerEvent &e)
  {
    // plt::figure(1);
    if (!have_local_traj_cable_ || !have_target_)
      return;
    ...
    {3, "uav swarm"}, 
    {4, "cable obstacle"}};
    std::vector<Eigen::VectorXd> results = planner_manager_->getResults();
    figure_.setResults(results);
    figure_.plot(ylabel_);
  }
  ...
  void EGOReplanFSM::polyTraj2ROSMsg(traj_utils::PolyTraj &msg)
  {
    ...
    void EGOReplanFSM::polyTraj2ROSMsg1(traj_utils::PolyTraj &msg)
  {
    auto data = &planner_manager_->traj_1.local_traj;
    msg.drone_id = 0;
    msg.traj_id = data->traj_id;
    msg.start_time = ros::Time(data->start_time);
    msg.order = 5; // todo, only support order = 5 now.
    ...
    void EGOReplanFSM::polyTraj2ROSMsg2(traj_utils::PolyTraj &msg)
  {
    auto data = &planner_manager_->traj_2.local_traj;
    msg.drone_id = 0;
    msg.traj_id = data->traj_id;
    msg.start_time = ros::Time(data->start_time);
    msg.order = 5; // todo, only support order = 5 now.
    ...
    void EGOReplanFSM::polyTraj2ROSMsg3(traj_utils::PolyTraj &msg)
  {
    auto data = &planner_manager_->traj_3.local_traj;
    msg.drone_id = 0;
    msg.traj_id = data->traj_id;
    msg.start_time = ros::Time(data->start_time);
    msg.order = 5; // todo, only support order = 5 now.
    ...
    void EGOReplanFSM::polyTraj2ROSMsg4(traj_utils::PolyTraj &msg)
  {
    auto data = &planner_manager_->traj_4.local_traj;
    msg.drone_id = 0;
    msg.traj_id = data->traj_id;
    msg.start_time = ros::Time(data->start_time);
    msg.order = 5; // todo, only support order = 5 now.
  ...
  bool EGOReplanFSM::callReboundReplan(bool flag_use_poly_init)
  {
    ...
    if (plan_success)
    {
      cable_success = callReboundReplanForCable(have_local_traj_cable_, desired_start_time);
    }

    cout << "cable success" << endl;

    double delta_replan_time = desired_start_time - ros::Time::now().toSec();
    ...
    {
        //publish traj
        traj_utils::PolyTraj msg1, msg2, msg3, msg4;
        polyTraj2ROSMsg1(msg1);
        polyTraj2ROSMsg2(msg2);
        polyTraj2ROSMsg3(msg3);
        polyTraj2ROSMsg4(msg4);

        poly_traj_pub_1.publish(msg1); //若轨迹规划成功则发布消息 to odometry
        poly_traj_pub_2.publish(msg2); //若轨迹规划成功则发布消息 to odometry
        poly_traj_pub_3.publish(msg3); //若轨迹规划成功则发布消息 to odometry
        poly_traj_pub_4.publish(msg4); //若轨迹规划成功则发布消息 to odometry
  ...
  bool EGOReplanFSM::callReboundReplanForCable(bool have_local, double start_time)
  {
    ...
    cout << "fenduan" << accs.cols() << endl;
    
    // 先装作无人机姿态角不变， 其实可以通过微分平坦算出来
    // bool plan_success = planner_manager_->ReboundReplanForCable_try1(accs, positions, durations);
    bool plan_success = planner_manager_->ReboundReplanForCable_try2(accs, positions, durations, have_local, start_time);
    return plan_success;
  }
