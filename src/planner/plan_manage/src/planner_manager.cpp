...
namespace ego_planner
{
  ...
  void EGOPlannerManager::initPlanModules(ros::NodeHandle &nh, PlanningVisualization::Ptr vis)
  {
    ...
    visualization_->setCablePoints(cable_load_->cable_points);
  ...
  bool EGOPlannerManager::ReboundReplanForCable_try1(Eigen::MatrixXd accs, Eigen::MatrixXd positions, Eigen::VectorXd durations)
  {
    std::vector< Eigen::Matrix<double,3,4> > uav_positions_total;
    bool opt_success = ploy_traj_opt_->OptimizeTrajectory_lbfgs_forCable0(accs, positions, durations, uav_positions_total);

    // for visualization trajectory
    if(opt_success)
    {
      traj_1.setLocalTraj(ploy_traj_opt_->getMinJerkOptPtr1()->getTraj(), ros::Time::now().toSec()); 
      traj_2.setLocalTraj(ploy_traj_opt_->getMinJerkOptPtr2()->getTraj(), ros::Time::now().toSec()); 
      traj_3.setLocalTraj(ploy_traj_opt_->getMinJerkOptPtr3()->getTraj(), ros::Time::now().toSec()); 
      traj_4.setLocalTraj(ploy_traj_opt_->getMinJerkOptPtr4()->getTraj(), ros::Time::now().toSec()); 
    }
    return opt_success;
  }

  bool EGOPlannerManager::ReboundReplanForCable_try2(Eigen::MatrixXd accs, Eigen::MatrixXd positions, Eigen::VectorXd durations, bool have_local, double trajectory_start_time)
  {
    Eigen::MatrixXd cable_coefs;
    size_t N = accs.cols();
    Eigen::Matrix3d init_state1, ifin_state1, init_state2, ifin_state2;
    Eigen::MatrixXd inner_state1, inner_state2;
    inner_state1.resize(3, N-2);
    inner_state2.resize(3, N-2);
    
    ploy_traj_opt_->getInitCable(accs, positions, durations, cable_coefs);
    if(have_local)
    {
      Eigen::Vector3d desired_start_pt =
          traj_cable_1.local_traj.traj.getPos(trajectory_start_time - traj_cable_1.local_traj.start_time);
      Eigen::Vector3d desired_start_vel =
          traj_cable_1.local_traj.traj.getVel(trajectory_start_time - traj_cable_1.local_traj.start_time);
      Eigen::Vector3d desired_start_acc =
          traj_cable_1.local_traj.traj.getAcc(trajectory_start_time - traj_cable_1.local_traj.start_time);
      init_state1 << desired_start_pt, desired_start_vel, desired_start_acc;
    }else{
      init_state1 << cable_coefs.block<3,1>(0,0), Eigen::Vector3d::Zero(), Eigen::Vector3d::Zero();
    }

    ifin_state1 << cable_coefs.block<3,1>(0,N-1), Eigen::Vector3d::Zero(), Eigen::Vector3d::Zero();
    for (size_t i = 1; i < N-1; i++)
    {
      inner_state1.col(i-1) = cable_coefs.block<3,1>(0,i);
    }

    ploy_traj_opt_->InitOptCable1(init_state1, ifin_state1, inner_state1, durations, trajectory_start_time);

    if(have_local)
    {
      Eigen::Vector3d desired_start_pt2 =
          traj_cable_2.local_traj.traj.getPos(trajectory_start_time - traj_cable_2.local_traj.start_time);
      Eigen::Vector3d desired_start_vel2 =
          traj_cable_2.local_traj.traj.getVel(trajectory_start_time - traj_cable_2.local_traj.start_time);
      Eigen::Vector3d desired_start_acc2 =
          traj_cable_2.local_traj.traj.getAcc(trajectory_start_time - traj_cable_2.local_traj.start_time);
      init_state2 << desired_start_pt2, desired_start_vel2, desired_start_acc2;
    }else{
      init_state2 << cable_coefs.block<3,1>(3,0), Eigen::Vector3d::Zero(), Eigen::Vector3d::Zero();
    }

    ifin_state2 << cable_coefs.block<3,1>(3,N-1), Eigen::Vector3d::Zero(), Eigen::Vector3d::Zero();
    for (size_t i = 1; i < N-1; i++)
    {
      inner_state2.col(i-1) = cable_coefs.block<3,1>(3,i);
    }
    ploy_traj_opt_->InitOptCable2(init_state2, ifin_state2, inner_state2, durations, trajectory_start_time);

    // // iterate 10 times
    Eigen::MatrixXd coef1, coef2;
    for (int i = 0; i < n_coef_iterate_; i++)
    {

      ploy_traj_opt_->optCable1(init_state1, ifin_state1, inner_state1, durations, trajectory_start_time);
      ploy_traj_opt_->optCable2(init_state2, ifin_state2, inner_state2, durations, trajectory_start_time);

      //obtain results last update
      ploy_traj_opt_->getLastOptCable(coef1, coef2);

      for (size_t i = 1; i < N-1; i++)
      {
        inner_state1.col(i-1) = coef1.col(i);
      }

      for (size_t i = 1; i < N-1; i++)
      {
        inner_state2.col(i-1) = coef2.col(i);
      }
      cout << i << endl;
    }

    // set local_traj of uavs
    ploy_traj_opt_->setUAVPosition(trajectory_start_time, have_local);
    // if(have_local)
    // {
      traj_1.setLocalTraj(ploy_traj_opt_->getMinJerkOptPtr1()->getTraj(), trajectory_start_time); 
      traj_2.setLocalTraj(ploy_traj_opt_->getMinJerkOptPtr2()->getTraj(), trajectory_start_time); 
      traj_3.setLocalTraj(ploy_traj_opt_->getMinJerkOptPtr3()->getTraj(), trajectory_start_time); 
      traj_4.setLocalTraj(ploy_traj_opt_->getMinJerkOptPtr4()->getTraj(), trajectory_start_time);
      traj_cable_1.setLocalTraj(ploy_traj_opt_->getMinJerkOptPtrCable1()->getTraj(), trajectory_start_time);
      traj_cable_2.setLocalTraj(ploy_traj_opt_->getMinJerkOptPtrCable2()->getTraj(), trajectory_start_time);
    // }else{
    //   traj_1.setLocalTraj(ploy_traj_opt_->getMinJerkOptPtr1()->getTraj(), ros::Time::now().toSec()); 
    //   traj_2.setLocalTraj(ploy_traj_opt_->getMinJerkOptPtr2()->getTraj(), ros::Time::now().toSec()); 
    //   traj_3.setLocalTraj(ploy_traj_opt_->getMinJerkOptPtr3()->getTraj(), ros::Time::now().toSec()); 
    //   traj_4.setLocalTraj(ploy_traj_opt_->getMinJerkOptPtr4()->getTraj(), ros::Time::now().toSec());
    //   traj_cable_1.setLocalTraj(ploy_traj_opt_->getMinJerkOptPtrCable1()->getTraj(), ros::Time::now().toSec());
    //   traj_cable_2.setLocalTraj(ploy_traj_opt_->getMinJerkOptPtrCable2()->getTraj(), ros::Time::now().toSec());
    // }

    return true;
  }

  std::vector<Eigen::VectorXd> EGOPlannerManager::getResults()
  {
    return ploy_traj_opt_->results;
  }
