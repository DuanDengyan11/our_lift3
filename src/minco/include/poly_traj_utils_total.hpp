...
namespace poly_traj
{
    ...
    public:
        ...
        inline Eigen::MatrixXd getAccs() const
        {
            ...
            for (int i = 0; i < N; i++)
            {
                accs.col(i) = pieces[i].getAcc(0.0);
            }
        ...
        inline double getLength()
        {
            ...
            double length = 0.0;
            for (int i=0; i<N; i++){
                Eigen::Vector3d vector = pieces[i].getPos(pieces[i].getDuration()) - pieces[i].getPos(0.0);
                length += vector.norm();
            }
    ...
    public:
        ...
        double start_world_time;

    private:
        int N; //piece num
        int m; //manipulation num
